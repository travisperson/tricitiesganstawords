  ...
  self.io.sockets.on('connection', function (socket) {
    ...
    socket.on('join_session', function (session) {
      ...
      if( self.sessions[session].pcount == 2 ) {
        ...
        delete self.sessions[session].dictionary
        self.sessions[session].playedWords = {}
        self.sessions[session].dictionary = new Dictionary // Reset the dictionary

        for ( var x in self.sessions[session].players)
          self.sessions[session].players[x].words = {}
          ...
          if (!game.playedWords.hasOwnProperty(word)) {
            ...
            Object.keys(game.players).forEach(function(_id) {
              if (id !== _id) {
                ...
                // Win / Lose
                if (Object.keys(game.players[_id].words).length >= 11) {
                  game.players[id].socket.emit('win', id)
                  game.players[_id].socket.emit('lose', _id)
                }
              }
